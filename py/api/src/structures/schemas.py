...
class OpenAISpeechRequest(BaseModel):
    ...
    model: str = Field(default="supertonic", description="TTS model to use")

    input: str = Field(..., description="The text to generate audio for", min_length=1)

    voice: str = Field(
        default="M1", description="Voice style to use (e.g., M1, F1, M2, F2)"
    )

    response_format: Literal["mp3", "opus", "aac", "flac", "wav", "pcm"] = Field(
        default="opus",
        description="Audio format. Opus and AAC recommended for streaming long text.",
    )
